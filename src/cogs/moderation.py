...
class Moderation(commands.Cog):
    ...
    @mod.command(name="move", description="Move a user from a voice channel.")
    @commands.has_permissions(move_members=True)
    @commands.bot_has_permissions(move_members=True)
    async def move(self, ctx, member: discord.Member, channel: discord.VoiceChannel):
        voice = member.voice
        if voice:
            user_role = member.top_role
            if ctx.author.top_role <= user_role:
                await ctx.respond(
                    f"{failed_emoji} Can't kick this user because your role is equal to or lower than their role.",
                    ephemeral=True,
                )
            ...
            if bot_role < user_role:
                await ctx.respond(
                    f"{failed_emoji} Can't kick this user because my role is below their role.",
                    ephemeral=True,
                )
                return
            await voice.move_to(channel)
            await ctx.respond(
                f"{success_emoji} Moved {member.mention} to {channel.mention}."
            )
        else:
            await ctx.respond(
                f"{failed_emoji} {member.mention} is not in a voice channel."
            )

    @mod.command(name="role", description="Give roles to users")
    ...
    async def role(self, ctx, member: discord.Member, role: discord.Role):
        ...
        if user_role >= ctx.author.top_role:
            await ctx.respond(
                f"{failed_emoji} Can't assign a role to a user whose role is greater or equal to yours.",
                ephemeral=True,
            )
            return
        ...
        if user_role >= bot_role:
            await ctx.respond(
                f"{failed_emoji} Can't assign a role to a user whose role is greater or equal to my role.",
                ephemeral=True,
            )
            return
        ...
        await ctx.respond(
            f"{success_emoji} Gave {member.mention} the {role.mention} role."
        )

    @mod.command(name="vkick", description="Kick a user from a voice channel.")
    @commands.has_permissions(kick_members=True)
    @commands.bot_has_permissions(kick_members=True)
    async def vkick(self, ctx, member: discord.Member):
        ...
        if voice:
            ...
            await ctx.respond(
                f"{success_emoji} Kicked {member.mention} from the voice channel."
            )
        else:
            await ctx.respond(
                f"{failed_emoji} {member.mention} is not in a voice channel."
    ...
    async def timeout(
        ...
        embed = discord.Embed(
            description=f"{success_emoji} {member.mention} has been timed out for {duration_string} by {ctx.author.mention}.\nReason: {reason}",
            color=int(embed_color[1:], 16),
        )
        await ctx.respond(embed=embed)

    @mod.command(name="untimeout", description="Untimeout a user.")
    @commands.has_permissions(moderate_members=True)
    async def untimeout(
        self,
        ctx,
        ...
        if member == ctx.author:
            embed = discord.Embed(
                description="You cannot untimeout yourself.",
                color=int(embed_color[1:], 16),
            )
        ...
        embed = discord.Embed(
            description=f"{success_emoji} {member.mention} has been untimeouted by {ctx.author.mention}.",
            color=int(embed_color[1:], 16),
        )
        await ctx.respond(embed=embed)
